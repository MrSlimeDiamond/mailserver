...
LIBFLAGS=-I/usr/include/json-c -ljson-c -I/usr/include/postgresql -lpq -lconfuse
DEBUG_CFLAGS=-g
...
# No plans to support Windows at this time, but the Makefile template has it lol
ifeq ($(OS), Windows_NT)
    # Windows specific
    BIN_EXT =.exe
    CFLAGS +=-lmingw32
...
# Check if the main exist in the src directory, if not prompt the user to enter the name of their target
ifeq ($(wildcard $(SRC_DIR)/$(MAIN).c),)
$(error Target source file '$(MAIN).c' not found in $(SRC_DIR). Please specify the target file using 'make TARGET=target_file_name' or edit the makefile)
endif
